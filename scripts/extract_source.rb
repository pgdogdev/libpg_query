...
runner.mock('get_attnum', :error_not_implemented) # TODO: We likely have to fix this to support row types (called from LookupTypeNameExtended)
runner.mock('get_atttype', :error_not_implemented) # TODO: We likely have to fix this to support row types (called from LookupTypeNameExtended)
runner.mock('recomputeNamespacePath', :do_nothing) # TODO: We likely have to fix this to support row types (called from LookupTypeNameExtended / TypenameGetTypidExtended)
runner.mock('TypenameGetTypidExtended') # TODO: We likely have to fix this to support row types (called from LookupTypeNameExtended)
...
#runner.mock('format_type_be', 'return pstrdup("-");')
runner.mock('build_row_from_class', :do_nothing)
runner.mock('get_collation_oid', 'return DEFAULT_COLLATION_OID;')
runner.mock('plpgsql_parse_wordtype', 'return NULL;')
runner.mock('plpgsql_parse_wordrowtype', 'return NULL;')
runner.mock('plpgsql_parse_cwordtype', 'return NULL;')
runner.mock('plpgsql_parse_cwordrowtype', 'return NULL;')
...
runner.mock('lookup_type_cache', :error_not_implemented) # This matters only for composite types, which are currently not supported
runner.mock('parse_datatype') # Required because this otherwise requires catalog access
